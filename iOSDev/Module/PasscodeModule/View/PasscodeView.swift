...
class PasscodeView: UIViewController {
    ...
    var passcodePresenter: PassscodePresenterProtocol!
    ...
    lazy var enterCodeAction = UIAction { [unowned self] sender in
        guard let sender = sender.sender as? UIButton else { return }
        self.passcodePresenter.enterPasscode(number: sender.tag)
    }
    
    lazy var deleteCodeAction = UIAction { [unowned self] sender in
        guard let sender = sender.sender as? UIButton else { return }
    ...
    override func viewDidLoad() {
        ...
        buttons.forEach {
            let btnLine = setHorizontalNumStack(range: $0)
            keyboardStack.addArrangedSubview(btnLine)
        }
...
extension PasscodeView {
    ...
    private func setHorizontalNumStack(range: [Int]) -> UIStackView {
        ...
        range.forEach {
            let numBtn = UIButton(primaryAction: enterCodeAction)
            numBtn.tag = $0
            numBtn.setTitle("\($0)", for: .normal)
            numBtn.setTitleColor(.white, for: .normal)
            numBtn.titleLabel?.font = UIFont.systemFont(ofSize: 60, weight: .light)
            numBtn.widthAnchor.constraint(equalToConstant: 60).isActive = true
